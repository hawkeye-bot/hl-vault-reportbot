...
`_render_chart` also overlays the ladder context onto the chart itself: recent buy fills (from a fresh `client.get_fills_since` scoped to the chart's own lookback window, so only fills actually visible on it are marked) as red upside-down triangles above each fill's candle (not at the exact fill price - reads as a "bought here" flag pointing down at the candle rather than sitting inside it; two-plus fills landing in the same candle stack their triangles upward instead of overwriting each other, via a separate `addplot` scatter series per stack level); each resting buy order as a dashed red line labeled with its rung number (`state.order_numbers`, so it matches `format_open_orders`'s "#" column); and resting sell order(s) as a dashed green line labeled "TP". Buy = red, sell = green throughout this overlay (`_BUY_COLOR`/`_SELL_COLOR` in `chart.py`) - deliberately the opposite of the candle body colors, which are about price direction, not order side. Stack/base offsets are sized against the *full* visible price range (candles plus the resting-order lines, which often sit well below/above the candles) rather than just the candle high/low, so they render as a sensible gap instead of collapsing into an overlapping blob when the order lines stretch the axis. This is the same chart used everywhere (buy fills, `/status`, heartbeat), so the overlay is identical regardless of which message it's attached to.
